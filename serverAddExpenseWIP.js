...
// Add expense ;;; needs to update group table total_expenses
// app.post('/currentTrips/addExpense', function(req, res) {
//
//   var name = req.body.memberName;
//   var amount = req.body.expenseAmount;
//   var group_id = req.body.expenseName;
// 	var insert_statement = "INSERT INTO transactions(group_id, member_id, amount, description) VALUES('" + group_id + "','" +
// 							name + "','" + amount + "');";
//   var update = "UPDATE total_expenses FROM groups WHERE group_id='" + group_id + "';";
//
...
app.post('/currentTrips/addExpense', function(req, res) {
  ...
  var amount = req.body.expenseAmount;
  var group_id = req.body.expenseName;
  var member_id = "SELECT member_id FROM members WHERE first_name='" + name + "';";
	var insert_statement = "INSERT INTO transactions(group_id, member_id, amount, description) VALUES('" + group_id + "','" +
							member_id + "','" + amount + "');";
  var update = "UPDATE total_expenses FROM groups WHERE group_id='" + group_id + "';";
  ...
  db.task('get-everything', task => {
        return task.batch([
            //task.any(member_id),
            task.any(insert_statement),  // update transactions
            task.any(update)   // update group.total_expenses
        ]);
    })
    .then(() => {
    	res.render('pages/home',{  // does not need to display, should just reload page
        // success
			})
    })
    .catch(error => {
